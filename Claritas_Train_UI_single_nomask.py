...
class NpyPatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"image": data["image"], "target": data["target"]}
...
# Training thread will be different to prevent freezing
    ...
    ######### Main Loss FUNCTİON ##########
    #######################################
    def adaptive_denoising_loss(self, output, target):
        ...
        return total_loss, mse.item(), ssim.item(), edge.item(), l1.item(), charbonnier.item() 
...
#####################################
    ...
    def get_file_pairs(self,noisy_dir,ref_dir):
        ...
        for x in sorted([os.path.join(noisy_dir, f) for f in os.listdir(noisy_dir) if f.endswith(".nrrd")]):
            ...
            datasett.append({"image": image_path, "target": target_path})
            k = k+1
        return datasett
    ...
    def prepare(self):
        ...
        # Warning, the inputs should be at least divisible by 32 to be safe. To prevent shape mismatch errors...
        ...
        val_transforms = Compose([
            SpatialPadd(keys=["image", "target"], spatial_size=tuple(int(s) for s in self.val_padsize.get().split(","))),
            CenterSpatialCropd(keys=["image", "target"],  roi_size=tuple(int(s) for s in self.val_cropsize.get().split(",")))  # Crop 3D patches
        ])

        # Get file names
        # Generate datasets
        train_ds = NpyPatchDataset(self.train_dir_noisy.get(), self.train_dir_ref.get(), transform=train_transforms)
        val_ds = NpyPatchDataset(self.val_dir_noisy.get(), self.val_dir_ref.get(), transform=val_transforms)
    ...
    def train_model(self,model, train_loader, val_loader, optimizer, num_epochs=50,pth_filename="best_pet_denoiser.pth"):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in train_loader:
                ...
                loss, mse, ssim, edge, l1, charbonnier =self.adaptive_denoising_loss(denoised_output, targets)
                loss.backward()
                optimizer.step()
...
#            self.print("Model saved...")
    ...
    def validate_model(self,model, val_loader):
        ...
        with torch.no_grad():
            for i, batch in enumerate(val_loader):
                ...
                total_l1 += l1
                total_charbonnier += charbonnier
                self.print(f"Individual Validation Loss:  Loss={loss.item():.6f}, MSE_Loss={mse:.6f}, SSIM_Loss={ssim:.6f},Edge_Loss={edge:.6f},L1_Loss={l1:.6f},Charbonnier Loss={charbonnier:.6f}")
    ...
    def evaluate_val_input_vs_target(self, val_loader=None, data_range=50.0):
        ...
        for batch in val_loader:
            ...
            loss, mse, ssim_loss, edge,l1,charbonnier= self.adaptive_denoising_loss(img, target)
            total_loss += loss.item()
            self.print(f"Original Individual Validation Loss:{loss.item():.6f}  MSE_Loss={mse:.6f}, SSIM_Loss={ssim_loss:.6f},Edge_Loss={edge:.6f},L1_Loss={l1:.6f},Charbonnier={charbonnier:.6f}")
            mse_total += mse
